...
if __name__ == "__main__":
    ...
    env = Env(network_file, train_file, max_delay,  conflict_delay_cap, early_cap)
    ...
    e = 0
    while True:
        f = open("20220402_MSE_result_softearlycap_epsilon-"+str(n_blocks)+"-"+str(n_max_trains)+".txt", 'a')
        episode_return = 0.0
        ...
        f.write("episode\t" + str(e) + "\t" + str(-1*env.current_lp_cost) + "\t" + str(env.last_max_train_end_delay) + "\t" + str(env.last_episode_return) + "\n")
        
        #agent.epsilon = max(agent.epsilon * agent.epsilon_decay, agent.epsilon_min)

        if e % 100 == 0:
            agent.model.save_weights (
                 "./save_model_softearlycap_epsilon/20220402_DQN_" + str (n_blocks) + "_" + str (n_max_trains) + "_" + str (
                     e) + ".weights.h5")
