...
class Env():
    ...
    def __init__(self, network_file, train_file, max_delay, conflict_delay_cap):
    ...
    def reset(self):
        ...
        # objective with "good sign": larger is better == -ag.cost
        ...
        # episode metrics for logging/plots (applied conflict delay tracking)
        self.max_conflict_increment = 0.0         # episode max per-block increment over all trains
        self.last_max_conflict_increment = float('nan')
        self.last_terminal_reward = float('nan')
        self.max_train_end_delay = 0.0              # episode max of final (end) delay across trains
        self.last_max_train_end_delay = float('nan')  # stash for logging on terminal
        self.episode_return = 0.0
        self.last_episode_return = float('nan')
    ...
    def step(self, action):
        ...
        if self.current_lp_cost == self.bigM:
            ...
            # episode return = sum of step rewards so far + terminal reward
        ...
        # (b) Per-block increments -> conflict cap (EVERY block increment ≤ cap, SOFT)
        starts, ends, max_incr_by_train, max_incr_global = ag.per_train_delay_increments(self.ag_graph, self.nodes)
        if starts is None:
            # negative cycle: terminate hard
            terminal_reward = -1 * self.bigM
            if hasattr(self, "last_max_train_end_delay"):
        ...
        # track episode max per-block increment (optional but useful)
        ...
        # SOFT CAP penalty for any exceedance over per-block cap
        over_cap = max(0.0, max_incr_global - self.conflict_delay_cap)
