...
class Env():
    ...
    def __init__(self, network_file, train_file, max_delay, conflict_delay_cap, early_cap, training=False):
        ...
        # final earliness penalty (max end-of-route earliness)
        self.w_end_early    = 50.0
        # cap for final earliness; reuse early_cap by default
        self.final_early_cap = self.early_cap
    ...
    def reset(self):
        ...
        ep = getattr(self, "episode_idx", 0)

      
        if getattr(self, "training", True):
            self.train_list = random.sample(self.train_ids, self.n_trains)
        else:
            self.train_list = list(self.train_ids)   
      
        if getattr(self, "training", True):
            if ep < 5000:
                self.conflict_delay_cap = 180
                self.early_cap = 180
            elif ep < 12000:
                self.conflict_delay_cap = 120
                self.early_cap = 120
            elif ep < 16000:
                self.conflict_delay_cap = 90
                self.early_cap = 60
            else:
                self.conflict_delay_cap = 120
                self.early_cap = 120
        ...
        if getattr(self, "training", True) and self.max_delay and self.max_delay > 0:
    ...
    def step(self, action):
        ...
        if self.current_lp_cost == self.bigM:
            ...
            if hasattr(self, "last_max_train_end_early"):
                self.last_max_train_end_early = getattr(self, "max_train_end_early", 0.0)
        ...
        # (a) First/last delays -> monotonicity + final earliness
        ...
        #  final earliness soft-cap penalty (based on end-of-train delays)
        if e_delays:
            min_end = min(e_delays)                    # most negative (earliest) end delay
            final_earliness = max(0.0, -float(min_end))  # positive magnitude in seconds
        else:
            final_earliness = 0.0

        # Track episode max final earliness
        if hasattr(self, "max_train_end_early"):
            if final_earliness > self.max_train_end_early:
                self.max_train_end_early = final_earliness

        cap_final_early = getattr(self, "early_cap", 60.0)  # reuse same cap as for increments
        over_final_early = max(0.0, final_earliness - cap_final_early)
        # uses w_early; if you want a different weight, add self.w_final_early in __init__
        r -= self.w_early * (over_final_early / max(1.0, float(cap_final_early)))
        ...
        # lateness soft-cap penalty (per-block increments)
        ...
        # earliness soft-cap penalty (per-block increments, symmetric)
