...
class Env():
    ...
    def __init__(self, network_file, train_file, max_delay, conflict_delay_cap, early_cap):
        ...
        self.early_cap = early_cap if early_cap is not None else 60.0  
    ...
    def reset(self):
        ...
        # easy -> target -> slightly harder -> back to target
        if ep < 5000:
            self.conflict_delay_cap = 1800   # easier late-cap
            self.early_cap          = 180    # easier early-cap
        elif ep < 12000:
            self.conflict_delay_cap = 1200   # target
            self.early_cap          = 120
        elif ep < 16000:
            self.conflict_delay_cap = 900    # briefly harder
            self.early_cap          = 60
        else:
            self.conflict_delay_cap = 1200   # back to target
            self.early_cap          = 120
    ...
    def step(self, action):
        ...
        # --- NEW: final earliness soft-cap penalty (based on end-of-train delays)
        # e_delays[i] < 0 means train i finishes early.
        if e_delays:
